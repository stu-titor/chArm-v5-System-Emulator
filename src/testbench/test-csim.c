...
int runtrace(int A, int B, int C, char *trace, /* in */
             int *ref_hits, int *ref_misses, int *ref_dirty_evictions,
             int *ref_clean_evictions, /* out */
             int *test_hits, int *test_misses, int *test_dirty_evictions,
             int *test_clean_evictions) /* out */
{
    FILE *fp;
    int status;
    char cmd[MAX_STR];
    ...
    sprintf(cmd, "./bin/csim-ref -A %d -B %d -C %d -t %s > /dev/null", A, B, C,
            trace);
    ...
    if (!fp) {
        fprintf(stderr, "Error: Results for reference simulator not found. Use "
                        "the printSummary() function\n");
        return 0;
    }
    status = fscanf(fp, "%d %d %d %d", ref_hits, ref_misses,
                    ref_dirty_evictions, ref_clean_evictions);
    fclose(fp);

    /* Run the test simulator */
    sprintf(cmd, "./bin/csim -A %d -B %d -C %d -t %s > /dev/null", A, B, C,
            trace);
    status = system("rm -rf .csim_results");
    if (status == -1) {
    ...
    status = fscanf(fp, "%d %d %d %d", test_hits, test_misses,
                    test_dirty_evictions, test_clean_evictions);
    fclose(fp);
...
void test_csim() {
    ...
    char *trace[N] = {"yi2.trace",   "yi.trace",    "yi.trace",
                      "dave.trace",  "trans.trace", "trans.trace",
                      "trans.trace", "trans.trace", "long.trace"};

    /* Output results */
    int ref_hits[N], ref_misses[N], ref_dirty_evictions[N],
        ref_clean_evictions[N];
    int test_hits[N], test_misses[N], test_dirty_evictions[N],
        test_clean_evictions[N];
    int status[N];
    int points[N];
    int total_points = 0;
    ...
    for (i = 0; i < N; i++) {
        ...
        status[i] =
            runtrace(A[i], B[i], C[i], trace_path, &ref_hits[i], &ref_misses[i],
                     &ref_dirty_evictions[i], &ref_clean_evictions[i],
                     &test_hits[i], &test_misses[i], &test_dirty_evictions[i],
                     &test_clean_evictions[i]);

        /* If the test had any problems, null everything out */
        if (status[i] == 0) {
            ref_hits[i] = ref_misses[i] = ref_dirty_evictions[i] =
                ref_clean_evictions[i] = -1;
            test_hits[i] = test_misses[i] = test_dirty_evictions[i] =
                test_clean_evictions[i] = -1;
        }
    }
    ...
    for (i = 0; i < N; i++) {
        ...
        if (status[i] != 0 && ref_hits[i] != -1 && ref_misses[i] != -1 &&
            ref_dirty_evictions[i] != -1 && ref_clean_evictions[i] != -1 &&
            test_hits[i] != -1 && test_misses[i] != -1 &&
            test_dirty_evictions[i] != -1 && test_clean_evictions[i] != -1) {
            points[i] += (ref_hits[i] == test_hits[i]) * weight[i];
            points[i] += (ref_misses[i] == test_misses[i]) * weight[i];
            points[i] +=
                (ref_dirty_evictions[i] == test_dirty_evictions[i]) * weight[i];
            points[i] +=
                (ref_clean_evictions[i] == test_clean_evictions[i]) * weight[i];
        }
    ...
    printf("%9s%8s%8s%8s%8s%8s|%8s%8s%8s%8s\n", "Points", "(A,B,C)", "Hits",
           "Misses", "DEvicts", "CEvicts", "Hits", "Misses", "DEvicts",
           "CEvicts");
    ...
    for (i = 0; i < N; i++) {
        ...
        printf("%8d%8d%8d%8d|%8d%8d%8d%8d  %s\n", test_hits[i], test_misses[i],
               test_dirty_evictions[i], test_clean_evictions[i], ref_hits[i],
               ref_misses[i], ref_dirty_evictions[i], ref_clean_evictions[i],
               trace[i]);
    }
    printf("%6d\n", total_points);
