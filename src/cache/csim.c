 ...
 */
void printSummary(int hits, int misses, int dirty_evictions,
                  int clean_evictions) {
    printf("hits:%d misses:%d dirty evictions:%d clean evictions:%d\n", hits,
           misses, dirty_evictions, clean_evictions);
    FILE *output_fp = fopen(".csim_results", "w");
    assert(output_fp);
    fprintf(output_fp, "%d %d %d %d\n", hits, misses, dirty_evictions,
            clean_evictions);
    fclose(output_fp);
}
...
void printUsage(char *argv[]) {
    ...
    printf("  -B <num>   Number of bytes per block. Must be >= 8 and a power "
           "of 2.\n");
...
int main(int argc, char *argv[]) {
    ...
    if (__builtin_popcountll(C / (A * B)) != 1) {
        printf("Invalid cache configuration; the number of sets must be a "
               "power of 2.\n");
        exit(1);
    }
    ...
    printSummary(hit_count, miss_count, dirty_eviction_count,
