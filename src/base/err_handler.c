...
static char *sevnames[LOG_FATAL + 2] = {"INFO", "WARNING", "ERROR", "FATAL",
                                        "OUTPUT"};

static char *sevcolors[LOG_FATAL + 2] = {
    ANSI_COLOR_CYAN, ANSI_COLOR_YELLOW, ANSI_COLOR_RED,
    ANSI_BOLD ANSI_COLOR_RED, ANSI_COLOR_GREEN};
...
static char *format_log_message(log_lev_t sev, char *msg) {
    assert(strlen(sevcolors[sev]) + strlen(sevnames[sev]) + strlen(msg) <
           BUF_LEN);
    sprintf(printbuf, "%s\t[%s] %s" ANSI_RESET, sevcolors[sev], sevnames[sev],
            msg);
    return printbuf;
}
