...
uint64_t loadElf(const char *fileName) {
    ...
    uintptr_t ptr =
        (uintptr_t) mmap(0, statBuffer.st_size, PROT_READ, MAP_PRIVATE, fd, 0);
    ...
    for (unsigned i = 0; i < entry_count; i++) {
        if (progHeader->p_type == PT_LOAD) {
            ...
            unsigned long f_align =
                filesz + (PAGESIZE - ((filesz + vaddr) % PAGESIZE));
            ...
            // Data initialization to 0 happens indirectly through calloc() call
            // in add_page().
            if (memsz > filesz) {
    ...
    Elf64_Shdr *sectionStrings =
        (Elf64_Shdr *) ((char *) sectionHeader +
                        (header->e_shstrndx * entry_size));
    ...
    for (unsigned i = 0; i < entry_count; i++) {
        ...
        // print
        sectionHeader =
            (Elf64_Shdr *) (((uintptr_t) sectionHeader) + entry_size);
    }
