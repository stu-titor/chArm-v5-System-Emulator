...
bool is_special_addr(const uint64_t addr) {
    return ((NULL_ADDR == addr) || (IO_CHAR_ADDR == addr) ||
            (RET_FROM_MAIN_ADDR == addr) || (CHECKPOINT_ADDR == addr));
}
...
static uint64_t _mem_read_special(const uint64_t addr, const unsigned width) {
    ...
    if (IO_CHAR_ADDR == addr) {
        ...
        switch (width) {
            case 1:
                scanf("%c\n", &data8);
                return (char) (data8 & 0xFFU);
                break;
            case 2:
                scanf("%hd\n", &data16);
                return (short) (data16 & 0xFFFFU);
                break;
            case 4:
                scanf("%d\n", &data32);
                return (int) (data32 & 0xFFFFFFFFU);
                break;
            case 8:
                scanf("%ld\n", &data64);
                return (long) data64;
...
static write_ret_code_t _mem_write_byte(const uint64_t addr,
                                        const uint8_t data) {
    uint64_t pnum = addr / PAGESIZE;
    uint64_t poff = addr % PAGESIZE;
    pte_ptr_t page = get_page(pnum);
...
static write_ret_code_t _mem_write_LE(const uint64_t addr, const uint64_t data,
...
static write_ret_code_t _mem_write_special(const uint64_t addr,
                                           const uint64_t data,
                                           const unsigned width) {
    if (NULL_ADDR == addr) {
...
static uint64_t _mem_read_cache(const uint64_t addr, const unsigned width) {
    ...
    for (size_t i = 0; i < width; i++) {
        ...
        if (!check_hit(guest.cache, current_address, READ)) {
            ...
            // hopefully they properly implemented writing incoming data to the
            // cache
            evicted_line_t *evicted =
                handle_miss(guest.cache, block_address, READ, block);
            data = *((uint64_t *) block + (addr & (B - 1)));
            // if the evicted line is valid and dirty, write changes back to
            // memory
            if (evicted->valid && evicted->dirty) {
                for (size_t j = 0; j < B; j++) {
                    _mem_write_LE(evicted->block_addr + j, evicted->data[j], 1);
                }
            }
            // free temps
            free(block);
...
static write_ret_code_t _mem_write_cache(const uint64_t addr,
                                         const uint64_t data,
                                         const unsigned width) {
    size_t B = guest.cache->B;
    uword_t current_address = addr;
    ...
    for (size_t i = 0; i < width; i++) {
        ...
        if (!check_hit(guest.cache, current_address, WRITE)) {
            ...
            evicted_line_t *evicted =
                handle_miss(guest.cache, block_address, WRITE, block);
...
write_ret_code_t _mem_write(const uint64_t addr, const uint64_t data,
                            const unsigned width) {
    if (is_special_addr(addr))
...
long long mem_read_LL(const uint64_t addr) {
    return (long long) _mem_read(addr, 8);
}

write_ret_code_t mem_write_B(const uint64_t addr, const char data) {
    return _mem_write(addr, (uint64_t) data, 1);
}
write_ret_code_t mem_write_S(const uint64_t addr, const short data) {
    return _mem_write(addr, (uint64_t) data, 2);
}
write_ret_code_t mem_write_I(const uint64_t addr, const int data) {
    return _mem_write(addr, (uint64_t) data, 4);
}
write_ret_code_t mem_write_L(const uint64_t addr, const long data) {
    return _mem_write(addr, (uint64_t) data, 8);
}
write_ret_code_t mem_write_LL(const uint64_t addr, const long long data) {
    return _mem_write(addr, (uint64_t) data, 8);
}
